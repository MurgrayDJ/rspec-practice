...
describe BinaryGame do
  ...
  describe '#player_input' do
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        ...
        invalid_num = "37"
        valid_input = "6"
        allow(game_input).to receive(:gets).and_return(invalid_word, invalid_num, valid_input)
      end
  ...
  describe '#display_turn_order' do
    ...
    let(:bin_search_turn) { instance_double(BinarySearch) }

    before do
      # You'll need to create a few method stubs.
      allow(bin_search_turn).to receive(:make_guess)
      allow(bin_search_turn).to receive(:update_range)
      allow(game_turn).to receive(:display_guess)
      allow(bin_search_turn).to receive(:game_over?) #From paragraph below
    end

    # Command Method -> Test the change in the observable state
    it 'increases guess_count by one' do
      expect { game_turn.display_turn_order(bin_search_turn) }.to change { game_turn.instance_variable_get(:@guess_count) }.by(1)
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends make_guess' do
      expect(bin_search_turn).to receive(:make_guess).and_return(5)
      game_turn.display_turn_order(bin_search_turn)
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends update_range' do
      expect(bin_search_turn).to receive(:update_range).once
      game_turn.display_turn_order(bin_search_turn)
    end
