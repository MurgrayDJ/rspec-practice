...
describe FindNumber do
  ...
  describe '#game_over?' do
    context 'when guess and random_number are equal' do
      ...
      subject(:correct_guess_game) { described_class.new(2, 3, random_number, 7)}
      ...
      it 'is game over' do
        expect(correct_guess_game).to be_game_over
      end
    end
    ...
    context 'when guess and random_number are not equal' do
      let(:random_number) { double('random_number', value: 7)}
      subject(:wrong_guess_game) { described_class.new(2, 3, random_number, 4)}
  ...
  describe '#update_range' do
    ...
    context 'when the guess is 7, min=5, and max=8' do
      subject(:good_guess_game) { described_class.new(5, 8, number_range, 7) }

      it 'updates min to the same value as max' do
        expect { good_guess_game.update_range }.to change { good_guess_game.min }.to(8)
      end

      it 'does not update max' do
        expect { good_guess_game.update_range }.not_to change { good_guess_game.max }
      end
    end
  end
end
